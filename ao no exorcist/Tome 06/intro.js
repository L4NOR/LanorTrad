...
function initializeChapterSelect() {
  ...
  if (selectMenu.options.length === 0) {
      ...
      var bonusChapters = ["Chapitre Bonus (1)", "Chapitre Bonus (2)"];
      for (var i = 0; i < bonusChapters.length; i++) {
          var option = document.createElement("option");
          option.value = "bonus" + (i + 1);
          option.text = bonusChapters[i];
          option.dataset.redirect = `https://lanortrad.netlify.app/ao%20no%20exorcist/tome%2006/${bonusChapters[i].toLowerCase().replace(' ', '%20')}`;
      ...
      for (var i = 23; i >= 20; i--) {
          ...
          option.dataset.redirect = `https://lanortrad.netlify.app/ao%20no%20exorcist/tome%2006/chapitre%20${formattedNumber}`;
